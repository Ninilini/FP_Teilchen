...
adc = pd.read_csv('plots/Grosser_Quellenscan/Cluster_adc_entries.txt', sep='\t', skiprows=1, names = ['{}'.format(i) for i in range(128)])
...
plt.savefig('plots/spectrum_adc.pdf')
plt.show()
plt.clf()
...
plt.savefig('plots/spectrum_keV.pdf')
plt.show()
plt.clf()
