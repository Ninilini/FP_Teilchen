...
counts = [0.0]*0
for i in U:
    adc = pd.read_csv('plots/CCE_Quelle/{}V_Cluster_adc_entries.txt'.format(i), skiprows=1, sep='\t', names = ['{}'.format(i) for i in range(128)] )#skwiprow: Zeile überspringen
    #adc = adc. values
    #print (adc)
...
plt.savefig('home/Janina/Documents/Physik/Praktikum/FP_Teilchen/Si_Streifensensor/Protokoll/plots/Effizienz.pdf')
plt.show()
